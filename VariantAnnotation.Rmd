...
ann$var.type <- sapply(vcf@info$TYPE, function(x) {
...
ov <- findOverlaps(vcf, coding, type = "equal")
ann$consequence <- rep(NA, nrow(ann))
x <- tapply(coding$CONSEQUENCE[subjectHits(ov)], queryHits(ov), function(x) {
    paste(sort(unique(x)), collapse = ",")
})
ann$consequence[as.integer(names(x))] <- x
with(ann, table(consequence, var.effect))
```
...
ann$percent.var <- ann$var.num / ann$depth * 100
# percentage of reads supporting ref
ann$percent.ref <- (ann$depth - ann$var.num) / ann$depth * 100
## 5.
...
ann$perc.ref.for <- vcf@info$SRF / ann$ref.num * 100
